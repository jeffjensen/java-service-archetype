// ── Test: single-service-ignores-areas ────────────────────────────────────────
// singleService=y  serviceAreas=orders,inventory
// Covers: when singleService=y, the serviceAreas property is silently ignored;
//         only the plain 'service' module is generated, regardless of serviceAreas content
...
// ── 2. Only plain 'service' exists — singleService=y ignores serviceAreas ────
...
assert !new File(basedir, 'service-orders').exists()    : "singleService=y must ignore serviceAreas; 'service-orders' must not be created"
assert !new File(basedir, 'service-inventory').exists() : "singleService=y must ignore serviceAreas; 'service-inventory' must not be created"
...
assert !parentPom.contains('<module>../service-orders</module>')    : "parent <modules> must not list service-orders"
assert !parentPom.contains('<module>../service-inventory</module>') : "parent <modules> must not list service-inventory"
...
['common-domain', 'domain-db-main', 'domain-rest', 'integration-db-main',
 'presentation-rest', 'service'].each { dep ->
    assert appPom.contains("<artifactId>${dep}</artifactId>") : "app missing dep: $dep"
}
assert !appPom.contains('<artifactId>service-orders</artifactId>')    : "app must not reference non-existent service-orders"
assert !appPom.contains('<artifactId>service-inventory</artifactId>') : "app must not reference non-existent service-inventory"
