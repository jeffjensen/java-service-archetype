...
// appName=No Integrations Test  integrations=  serviceAreas=,  presentationTypes=rest
...
def aid = 'no-integrations-test'
def appName = 'No Integrations Test'
def raw = { String rel ->
...
def modules = [
    ...
    'domain-service',
    'domain-rest', 'presentation-rest', 'service',
]
modules.each { m -> check("${m}/pom.xml") }

// ── 3. No integration modules generated ──────────────────────────────────────

assert !new File(basedir, 'domain-db-main').exists()         : "No integration modules should be created"
assert !new File(basedir, 'integration-db-main').exists()    : "No integration modules should be created"
...
['common-domain', 'domain-rest', 'presentation-rest', 'service'].each { dep ->
...
assert !appPom.contains('<artifactId>integration-db-main</artifactId>')  : 'app must not reference non-existent integration module'
// no database integration: no spring-boot-starter-data-jpa; web/actuator/admin stack unconditional
...
['app', 'common-domain', 'domain-rest'].each { dep ->
...
// ── AppServletInitializer is unconditional now (app is always a servlet web app) ─

check("app/src/main/java/${p}/app/config/AppServletInitializer.java")
assert !text("app/src/main/java/${p}/app/config/Application.java").contains('@EnableTransactionManagement') \
    : 'no database integration: Application must not @EnableTransactionManagement'
